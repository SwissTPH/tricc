...
```
tricc_oo/
Γö£ΓöÇΓöÇ converters/      # Format conversion logic (drawioΓåÆtricc, triccΓåÆoutputs)
Γö£ΓöÇΓöÇ models/          # Pydantic data models (nodes, edges, operations)
Γö£ΓöÇΓöÇ parsers/         # XML parsing for draw.io files
Γö£ΓöÇΓöÇ serializers/     # Output format serialization
Γö£ΓöÇΓöÇ strategies/      # Input/output strategy implementations
Γö£ΓöÇΓöÇ tools/           # Draw.io templates and utilities
ΓööΓöÇΓöÇ visitors/        # Graph traversal and processing
feature/             # Feature specs (business + technical); Draft ΓåÆ Approved gate
...
### Step 1 ΓÇö Feature specification (`feature/`)

1. Create `feature/<feature-name>.md` (or update an existing spec).
2. Structure the document in **two parts**:
   - **Part I ΓÇö Business description** ΓÇö for clinical authors, guideline developers, and implementers evaluating workflows. Plain language, examples, benefits, limitations. No file paths or function names unless unavoidable.
   - **Part II ΓÇö Technical specification** ΓÇö for developers: formal semantics, pipeline, code checklist, tests, acceptance criteria, implementation phases.
3. Set the status table at the top to **`Draft`**.
4. Do **not** implement until the spec is approved.

### Step 2 ΓÇö User approval
...
**Status values:** `Draft` ΓåÆ `Approved` ΓåÆ `Implemented` ΓåÆ `Superseded`
...
- Keep per-process/segment libraries **thin** ΓÇö they should mostly contain named `define` statements that delegate to the Helper.
