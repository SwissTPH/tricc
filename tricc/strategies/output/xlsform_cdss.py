...
class XLSFormCDSSStrategy(XLSFormStrategy):
    ...
    def process_export(self, start_pages, **kwargs):
        ...
        if len(diags) > 0:
            ...
            self.df_survey.loc[len(self.df_survey)] = get_diagnostic_add_line(
                diags, self.df_choice
            )
    ...
    def export_diag(self, activity, diags=[], **kwargs):
        for node in activity.nodes.values():
            ...
            if hasattr(node, "name") and node.name is not None:
                if (
                    node.name.startswith("diag")
                    and node.last
                    and not any(
                        [
                            get_export_name(diag) == get_export_name(node)
                            for diag in diags
                        ]
                    )
                ):
